...
const Dashboard = () => {
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <h2 className="text-2xl font-semibold text-gray-800 mb-2">No Bill Data Found</h2>
          <p className="text-gray-600 mb-6">Please upload a bill or enter details first.</p>
          <button
            onClick={() => navigate('/')}
  ...
  const payableAmount = parseFloat(billData?.payableWithinDueDate || billData?.totalCharges) || 0;
  ...
  if (units > 0 && tariffRate > 0) {
    ...
    totalExtraCharges = fuelAdjustment + fcSurcharge + quarterlyAdjustment + meterRent + serviceRent;
  ...
  const breakdownData = [
    ...(baseUsage > 0 ? [{ name: 'Base Usage', value: baseUsage, color: '#3B82F6' }] : []),
    ...(taxes > 0 ? [{ name: 'Taxes (5%)', value: taxes, color: '#F59E0B' }] : []),
    ...(totalExtraCharges > 0 ? [{ name: 'Extra Charges', value: totalExtraCharges, color: '#EF4444' }] : [])
  ];

  const detailedBreakdownData = [
    ...(baseUsage > 0 ? [{ name: 'Base Electricity Cost', value: baseUsage, color: '#3B82F6' }] : []),
    ...(fuelAdjustment > 0 ? [{ name: 'Fuel Price Adjustment', value: fuelAdjustment, color: '#8B5CF6' }] : []),
    ...(fcSurcharge > 0 ? [{ name: 'FC Surcharge', value: fcSurcharge, color: '#EC4899' }] : []),
    ...(quarterlyAdjustment > 0 ? [{ name: 'Quarterly Adjustment', value: quarterlyAdjustment, color: '#14B8A6' }] : []),
    ...(meterRent > 0 ? [{ name: 'Meter Rent', value: meterRent, color: '#F97316' }] : []),
    ...(serviceRent > 0 ? [{ name: 'Service Rent', value: serviceRent, color: '#6B7280' }] : []),
    ...(taxes > 0 ? [{ name: 'Taxes (5%)', value: taxes, color: '#F59E0B' }] : [])
  ];
        ...
        {!hasEssentialData && (
          <div className="mb-6 bg-yellow-50 border-l-4 border-yellow-400 p-4 rounded-lg">
            <div className="flex items-start">
              ...
              <div>
                <h3 className="text-sm font-medium text-yellow-800">Incomplete Bill Data</h3>
                <p className="text-sm text-yellow-700 mt-1">
                  Some essential information could not be extracted from your bill. 
                  Please switch to manual entry for accurate analysis.
                </p>
                <button
                  onClick={() => navigate('/')}
                ...
                <div className="space-y-4">
                  {units > 0 && (
                    <div className="p-4 bg-blue-50 rounded-lg">
                      <div className="flex items-center justify-between">
                        <span className="text-sm text-gray-600">Units Consumed</span>
                      ...
                      <div className="mt-2 w-full bg-blue-200 rounded-full h-2">
                        <div 
                          ...
                          style={{ width: `${Math.min(((simulatedBill?.units || units) / 50) * 100, 100)}%` }}
                        ></div>
                      </div>
                  ...
                  <div className="border-t pt-4">
                    <h4 className="text-sm font-medium text-gray-700 mb-2">Rate Analysis</h4>
                    <div className="space-y-2">
                      {tariffRate > 0 && (
                        <div className="flex justify-between text-sm">
                          <span className="text-gray-600">Tariff Rate:</span>
                          <span className="font-medium">Rs {tariffRate.toFixed(2)}/unit</span>
                        </div>
                      )}
                      {units > 0 && totalBill > 0 && (
                        <div className="flex justify-between text-sm">
                          <span className="text-gray-600">Effective Rate:</span>
                          <span className="font-medium">Rs {(totalBill / units).toFixed(2)}/unit</span>
                        </div>
                      )}
                    </div>
                  </div>
          ...
          <div className="mt-8 grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="bg-white rounded-xl shadow-md p-6">
              <h3 className="text-lg font-semibold text-gray-800 mb-4 flex items-center">
                ...
                Detailed Bill Breakdown {billData?.company ? `(${billData.company})` : ''}
              </h3>
              <BreakdownTable 
